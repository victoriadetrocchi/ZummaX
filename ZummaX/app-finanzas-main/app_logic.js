...
async function loadDashboardData() { 
    ...
    try {
        ...
        if (response.ok) {
            ...
            // Corregido: El ID de ingresos era duplicado en el HTML de dashboard.
            // Asumamos que el de la cabecera es el único 'ingresosTotal'
...
async function loadGraficoCategorias() { 
    ...
    if (!ctx) return;
    try {
        const response = await fetch(`${API_URL}/graficos/categorias`, { method: 'GET', headers: { 'Authorization': `Bearer ${token}`, cache: 'no-cache' }});
        if (response.ok) {
            const result = await response.json();
            ...
            new Chart(ctx, { type: 'pie', data: { labels: result.labels, datasets: [{ data: result.data, backgroundColor: ['#FF6384','#36A2EB','#FFCE56','#4BC0C0','#9966FF','#FF9F40'] }] }, options: { responsive: true, plugins: { legend: { position: 'bottom' }}}});
        } else if (response.status === 401) { alert('Sesión expirada.'); window.location.href = 'login.html'; }
        else { throw new Error('Error al cargar gráfico de categorías'); }
...
async function loadGraficoEvolucion(periodo = 'mensual') {
    ...
    try {
        const response = await fetch(`${API_URL}/graficos/evolucion?periodo=${periodo}`, { method: 'GET', headers: { 'Authorization': `Bearer ${token}`, cache: 'no-cache' }});
        if (!response.ok) {
            if (response.status === 401) { alert('Sesión expirada.'); window.location.href = 'login.html'; }
        ...
        const chartData = {
            ...
            datasets: [
                { label: 'Ingresos', data: result.ingresos, backgroundColor: 'rgba(75, 192, 192, 0.6)', borderColor: 'rgba(75, 192, 192, 1)', borderWidth: 1 },
                { label: 'Egresos', data: result.egresos, backgroundColor: 'rgba(255, 99, 132, 0.6)', borderColor: 'rgba(255, 99, 132, 1)', borderWidth: 1 }
            ]
        };
        const chartOptions = { responsive: true, scales: { y: { beginAtZero: true } }, plugins: { legend: { position: 'top' } } };
